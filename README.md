...
> Notion을 활용한 <strong>WBS(Work Breakdown Structure)</strong>와 요구사항 명세를 통해 역할을 분담하고, 각 팀원의 진행상황을 한눈에 파악할 수 있도록 구성했습니다.
...
- 트러블 슈팅 요약
  > /api 경로를 명시했기 때문에, 프론트에서 요청 시 URL에 반드시 /api를 포함해야 동작함
...
> 변경된 summaryItems를 깊은 감시(deep watch)로 감지하여 적절히 리렌더링
...
> 📌 deep: true는 객체 내부의 속성까지 감지하게 해주며, immediate: true는 컴포넌트가 mount될 때도 즉시 실행되도록 해줍니다.
...
수입/지출에 따른 카테고리별 금액 합계를 파이 차트로 시각화하려 했지만,  
화면에 차트가 렌더링되지 않는 문제 발생

##### 🧩 원인

`chart.js` + `vue-chartjs`에서는 `labels`와 `data` 배열의 길이가 다를 경우  
차트가 렌더링되지 않거나 오류가 발생함

- `labels`: 항상 10개의 고정된 카테고리명
- `data`: 합계가 0인 항목은 제외되면서 길이가 가변적  
  → 이로 인해 차트가 렌더링되지 않음
...
- `fileToBase64`로 텍스트 변환 후 `imgUrl`에 재할당하여 해결
...
- `PaymentDetail.vue`에서 상세 조회 시 데이터가 불러와지지 않음
...
- `Number()`를 사용하여 숫자형으로 변환 후 전달

```js
// ⚠️ 오류 코드
const route = useRoute()
const paymentId = ref(route.params.id) // 문자열로 전달됨

<PaymentDetailList :id="paymentId" />

// ✅ 수정 코드
const paymentId = ref(Number(route.params.id)) // 숫자로 변환하여 전달
<PaymentDetailList :id="paymentId" />
```
...
> **첫 협업을 경험하며**  
> 개발도 협업도 모두 처음이라 어려운 점이 많았지만, 작은 것부터 하나하나 물어보며 진행한 덕분에 개발 흐름을 익히고 팀 프로젝트가 어떻게 흘러가는지 경험할 수 있었습니다. 질문이 많았던 저를 친절하게 도와주신 팀장님과 팀원분들께 정말 감사드리고, 제 코드로 인해 혹시라도 충돌이 발생해 다른 분들의 작업에 영향을 줄까 봐 Git 머지가 가장 두려웠던 순간이기도 했습니다.  
...
- **원인**: `SearchBar.vue`에서 `emit()` 시 `searchType`, `searchKeyword` 누락
- **해결 방법**: `emit()`에 항상 `{ type, keyword, categories }`를 함께 전달하도록 수정
